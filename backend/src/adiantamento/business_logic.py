        ...
        if pd.notna(inicio_afast):
            ...
            # REGRAS 2.4, 2.7 e 2.8: FÉRIAS
                ...
                if (
                    ...
                    status = "Inelegível"
                    observacoes.append(
                        f"Férias iniciam dia {inicio_afast.day} - após dia 15 (não trabalhou primeira quinzena completa)"
                    )
                ...
                # CASO ESPECIAL: Férias começam e terminam no mesmo mês (ex: Vicente 6/10 a 10/10)
                    ...
                    dias_antes_ferias = (
                        inicio_afast.day - 1
                    )  # dias 1 até dia antes das férias
                    dias_depois_ferias = (
                        dias_no_mes - fim_afast.day
                    )  # dias após retorno até fim do mês
                    ...
                    if dias_trabalhados_primeira_quinzena == 0:
                        status = "Inelegível"
                        observacoes.append(
                            f"Férias {inicio_afast.day}/{mes} a {fim_afast.day}/{mes} - não trabalhou na primeira quinzena"
                        )
                    else:
                        observacoes.append(
                ...
                # REGRA 2.8: Retorno de férias dia 16+ não recebe
                    ...
                    status = "Inelegível"
                    observacoes.append(
                        f"Retorno de férias após dia {pay_day} ({fim_afast.strftime('%d/%m/%Y')})"
                    )
                ...
                # REGRA 2.4: Retorno de férias entre 1 e (pay_day-1) - proporcional
                    ...
                    if fim_afast.day >= 15:
                        status = "Inelegível"
                        observacoes.append(
                            f"Retorno de férias dia {fim_afast.day + 1}/{mes} - não trabalhou na primeira quinzena"
                        )
                    else:
                        # Dias trabalhados após o retorno
                        dias_a_considerar = dias_no_mes - fim_afast.day
                        observacoes.append(
                            f"Retorno de férias dia {fim_afast.day + 1}/{mes} ({dias_a_considerar} dias)"
                        )
                ...
                # REGRA 2.6: Início de férias entre 1 e (pay_day-1) - proporcional
                    ...
                    if inicio_afast.day <= 1:
                        status = "Inelegível"
                        observacoes.append(
                            f"Férias desde dia {inicio_afast.day} - não trabalhou na primeira quinzena"
                        )
                    # REGRA 1.4: Se tem consignado, recebe proporcional; se não tem, é inelegível
                        ...
                        observacoes.append(
                            f"Férias proporcional até {inicio_afast.strftime('%d/%m/%Y')} ({dias_a_considerar} dias)"
                        )
                ...
                # REGRA 2.7: Férias começam no dia do pagamento (dia 15)
                    ...
                    observacoes.append(
                        f"Férias iniciam dia {inicio_afast.day} (no dia do pagamento)"
                    )
                ...
                if rule.code == "REMBRAZ":
                    if inicio_afast and fim_afast:
                        if inicio_afast.day != 1 or fim_afast.day != 30:
                            observacoes.append(
                                f"⚠️ ALERTA: Férias não cobrem período completo (1-30)"
                            )
...
# --- NOVA FUNÇÃO MODIFICADA (PROPORCIONALIDADE REAL) ---
def aplicar_descontos_consignado(
    df_calculado: pd.DataFrame, rule: CompanyRule
) -> pd.DataFrame:
    """
    Calcula o desconto do consignado de forma matemática e proporcional.
    Se o funcionário receber 20% do salário em adiantamento, desconta 20% da parcela.
    Se receber 40%, desconta 40%.
    """
    df_final = df_calculado.copy()

    # 1. Sanitização Mínima de Segurança (Apenas para colunas de cálculo)
    cols_to_check = [
        "ValorParcelaConsignado",
        "SalarioContratual",
        "ValorAdiantamentoBruto",
    ...
    # 2. Cálculo do Fator Efetivo de Desconto
    # Fator = Valor Bruto do Adiantamento / Salario Contratual
    df_final["FatorDesconto"] = 0.0

    # Cenário A: Salário maior que zero (Cálculo Proporcional Real)
    mask_salario_ok = df_final["SalarioContratual"] > 0
    df_final.loc[mask_salario_ok, "FatorDesconto"] = (
        df_final.loc[mask_salario_ok, "ValorAdiantamentoBruto"]
        / df_final.loc[mask_salario_ok, "SalarioContratual"]
    )

    # Cenário B: Fallback de Segurança (Se salário for 0, usa o % do cadastro para não zerar desconto)
    mask_salario_zero = (df_final["SalarioContratual"] == 0) & (
        df_final["PercentualAdiant"] > 0
    )
    df_final.loc[mask_salario_zero, "FatorDesconto"] = (
        df_final.loc[mask_salario_zero, "PercentualAdiant"] / 100.0
    )

    # Arredondamento do Fator para 2 casas decimais (Padrão Fortes, ex: 0.399 -> 0.40)
    df_final["FatorDesconto"] = (df_final["FatorDesconto"] * 100).round(2) / 100

    # 3. Cálculo do Valor do Desconto
    valor_desconto = df_final["ValorParcelaConsignado"] * df_final["FatorDesconto"]

    # Regra Específica: BEL MICRO não desconta consignado no adiantamento
    if rule.overrides.get("consignado_provision_pct") == 0.0:
        valor_desconto[:] = 0.0

    # Arredondamento do Valor Monetário do Desconto
    if not rule.overrides.get("no_rounding", False):
    ...
    # Garante que o líquido não fique negativo
    df_final.loc[
        df_final["ValorLiquidoAdiantamento"] < 0, "ValorLiquidoAdiantamento"
    ] = 0

    # 5. Formatação da Observação (Exibe o % real aplicado)
    ...
    def append_obs(row):
        ...
        pct_aplicado = row["FatorDesconto"] * 100

        # Formatação clara: Parcela X | Desc: Y a Z%
        new_obs = f"Consignado (Parcela: R${parcela:,.2f} | Desc: R${desconto:,.2f} a {pct_aplicado:.2f}%)"
    ...
    df_final.drop(columns=["FatorDesconto"], inplace=True)
