...
from .fortes_corrections import FortesCorrection
from .fortes_pre_calculation import FortesPreCalculation, calcular_parametros_correcao
from .fortes_auto_recalc import FortesAutoRecalc
...
class AdiantamentoWorkflow:
    ...
    def executar_calculo(self) -> WorkflowResult:
        ...
        """
        try:
            log.info(f"Iniciando cálculo para {self.rule.name} - {self.mes:02d}/{self.ano}")
    ...
    def aplicar_correcoes_fortes(
        ...
        try:
            ...
            log.warning("APÓS estas alterações, você DEVE RECALCULAR a folha no Fortes!")
            log.warning("=" * 80)
            ...
            if auto_recalc and fortes_user and fortes_password_hash:
                ...
                recalc = FortesAutoRecalc(self.empresa_codigo, fortes_user, fortes_password_hash)
                ...
                if result_recalc['sucesso']:
                    ...
                    log.warning("⚠️ AÇÃO NECESSÁRIA: Recalcule a folha manualmente no Fortes!")
            else:
    ...
    def gerar_relatorios(
        ...
        try:
            ...
            if not caminho_pdf and not caminho_csv:
                return WorkflowResult(
                    status=WorkflowStatus.ERRO,
                    mensagem="Falha ao gerar relatórios"
                )
    ...
    def executar_fluxo_completo(
        ...
        if result.status in [WorkflowStatus.AGUARDANDO_CONFIRMACAO, WorkflowStatus.ERRO]:
            return result
