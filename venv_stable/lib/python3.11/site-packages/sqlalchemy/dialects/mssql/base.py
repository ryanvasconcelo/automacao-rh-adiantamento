...
* When RETURNING is not available or has been disabled via
  ...
  * Other dialects such as pyodbc will call upon
...
* ``AUTOCOMMIT`` - pyodbc / pyodbc-specific
* ``READ COMMITTED``
* ``READ UNCOMMITTED``
* ``REPEATABLE READ``
* ``SERIALIZABLE``
...
* The flag can be set to either ``True`` or ``False`` when the dialect
  is created, typically via :func:`_sa.create_engine`::
        ...
        eng = create_engine("mssql+pyodbc://user:pass@host/db",
...
.. sourcecode:: pycon+sql
    ...
    >>> eng = create_engine("mssql+pyodbc://mydsn", legacy_schema_aliasing=True)
...
class BIT(sqltypes.Boolean):
    ...
    Both pyodbc and pyodbc return values from BIT columns as
